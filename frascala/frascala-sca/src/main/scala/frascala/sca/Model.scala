...
package frascala.sca {
    ...
    trait ScaIntentController extends AnnotationController {
        ...
        protected def require = intents.reduce("")({ (acc, intent) => acc+" "+intent.name })
    }
    ...
    trait ScaPrimitiveProperty[T] extends ScaProperty[T] {
        ...
        def toXML = if (value != null) <property name={ name } source={ source map { "$"+_.name } orNull } type={ xsdType } xmlns:xsd="http://www.w3.org/2001/XMLSchema">{ value.toString }</property>
        else
    ...
    trait ScaConstrainingTypeeMembrane extends ScaCompositeMembrane {
        ...
        override def toDocument = <constraingType xmlns="http://www.osoa.org/xmlns/sca/1.0" name={ name } requires={ require }>
                                      { ports map { _.toXML } }
                                      { properties map { _.toXML } }
                                      { components map { _.toXML } }
                                      { connectors map { _.toXML } }
